...
def representative_dataset():
    for data in tf.keras.datasets.cifar10:
        yield {
            "image": data.image,
            "bias": data.bias,
        }
...
end_step = np.ceil(num_images / batch_size).astype(np.int32) * epochs
pruning_params = {
    'pruning_schedule': tfmot.sparsity.keras.PolynomialDecay(initial_sparsity=0.0, final_sparsity=0.90, begin_step=0,
                                                             end_step=end_step)}
...
callbacks = [tfmot.sparsity.keras.UpdatePruningStep()]
removed_filters_model.fit(train_images, train_labels, batch_size=batch_size, epochs=epochs,
                          validation_data=(test_images, test_labels), callbacks=callbacks)
...
print(
    "Size of zipped baseline pruned Keras model: %.2f MB" % (get_zipped_model_size("mbnv1_pruned.h5", "mbnv1_pruned")))
print(
    "Size of zipped baseline stripped Keras model: %.2f MB" % (get_zipped_model_size("mbnv1_strip.h5", "mbnv1_strip")))
print("Size of zipped baseline removed Keras model: %.2f MB" % (
    get_zipped_model_size("mbnv1_removed_filters.h5", "mbnv1_removed_filters")))
print()
...
print("Size of zipped removed TFLite model: %.2f MB" % (
    get_zipped_model_size("mbnv1_removed_filters.tflite", "baseline")))
print()
print("Size of zipped baseline TFLite quantized model: %.2f MB" % (
    get_zipped_model_size("mbnv1_baseline_optim.tflite", "baseline")))
print("Size of zipped stripped TFLite quantized model: %.2f MB" % (
    get_zipped_model_size("mbnv1_stripped_optim.tflite", "baseline")))
print("Size of zipped removed TFLite quantized model: %.2f MB" % (
    get_zipped_model_size("mbnv1_removed_filters_optim.tflite", "baseline")))
print()
